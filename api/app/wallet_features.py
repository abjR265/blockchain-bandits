"""Wallet feature engineering for inference (mirrors `ml/src/blockchain_bandits_ml/features.py`).

Keep FEATURE_COLUMNS and formulas in sync with the ML training package.
"""
...
def _mev_address_set() -> frozenset[str]:
    candidates = [
        ...
        Path(__file__).resolve().parents[2] / "data" / "label_sources" / "mev_addresses.csv",
    ]
    for p in candidates:
        if not p.is_file():
            continue
...
def engineer(txs: pd.DataFrame, now_ts: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    for w in wallets:
        ...
        tx_count_out = len(tx_out)
        tx_count_in = len(tx_in)
...
def feature_row_for_address(
    address: str,
    *,
    etherscan_api_key: str | None = None,
    allow_synthetic: bool = False,
) -> pd.Series:
    """Build feature vector for `address`.

    - With `etherscan_api_key`: mainnet tx list from Etherscan (live).
    - With `allow_synthetic=True` only: deterministic fake txs (demo / tests).
    - Otherwise callers must supply an API key or this raises `EtherscanError`.
    """
    from app.config import get_settings
    from app.etherscan import EtherscanError, fetch_txlist_mainnet

    addr = address.lower()
    if etherscan_api_key and etherscan_api_key.strip():
        chain_id = get_settings().etherscan_chain_id
        txs = fetch_txlist_mainnet(addr, etherscan_api_key.strip(), chain_id=chain_id)
    elif allow_synthetic:
        txs = synthetic_transactions_for_address(addr)
    else:
        raise EtherscanError(
            "Set ETHERSCAN_API_KEY for live mainnet data, or ALLOW_SYNTHETIC_FEATURES=true for demo-only synthetic transactions."
        )

    df = engineer(txs)
    row = df.loc[df["address"] == addr]
    if row.empty:
